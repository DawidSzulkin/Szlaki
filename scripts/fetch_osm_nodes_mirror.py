...
def fetch_from_mirror():
    # Używamy alternatywnego mirrora zamiast głównego api.de
    overpass_url = "https://overpass.kumi.systems/api/interpreter"
    bbox = "49.5,18.9,50.0,20.2"
    nodes = []

    headers = {
        'User-Agent': 'Mozilla/5.0 (Windows NT 10.0; Win64; x64) SzlakiApp/1.0',
    ...
    if nodes:
        ...
        print(f"\nSukces! Zapisano {len(nodes)} węzłów do {OUTPUT_PATH.name}")
    else:
        print("\nBŁĄD: Mirror również odrzucił zapytania. Problem leży w lokalnej sieci lub restrykcjach IP.")
